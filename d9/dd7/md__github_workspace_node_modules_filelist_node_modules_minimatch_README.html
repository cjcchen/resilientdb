...
<title>ResilientDB: minimatch</title>
...
<script type="text/javascript">
/* @license magnet:?xt=urn:btih:d3d9a9a6595521f9666a5e94cc830dab83b65699&amp;dn=expat.txt MIT */
$(document).ready(function(){initNavTree('d9/dd7/md__github_workspace_node_modules_filelist_node_modules_minimatch_README.html','../../'); initResizable(); });
/* @license-end */
</script>
<div id="doc-content">
...
<div class="contents">
<div class="textblock"><p>A minimal matching utility.</p>
<p><a href="http://travis-ci.org/isaacs/minimatch"><img src="https://travis-ci.org/isaacs/minimatch.svg?branch=master" alt="Build Status" style="pointer-events: none;" class="inline"/></a></p>
...
<h1><a class="anchor" id="autotoc_md1459"></a>
Usage</h1>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md1460"></a>
Features</h1>
...
<p>Though windows uses either <code>/</code> or <code>\</code> as its path separator, only <code>/</code> characters are used by this glob implementation. You must use forward-slashes <b>only</b> in glob expressions. Back-slashes in patterns will always be interpreted as escape characters, not path separators.</p>
<p>Note that <code>\</code> or <code>/</code> <em>will</em> be interpreted as path separators in paths on Windows, and will match against <code>/</code> in glob expressions.</p>
<p>So just always use <code>/</code> in patterns.</p>
<h1><a class="anchor" id="autotoc_md1462"></a>
Minimatch Class</h1>
...
<li><code>set</code> A 2-dimensional array of regexp or string expressions. Each row in the array corresponds to a brace-expanded pattern. Each item in the row corresponds to a single path-part. For example, the pattern <code>{a,b/c}/d</code> would expand to a set of patterns like: <pre class="fragment">  [ [ a, d ]
  , [ b, c, d ] ]
</pre> If a portion of the pattern doesn't have any "magic" in it (that is, it's something like <code>"foo"</code> rather than <code>fo*o?</code>), then it will be left as a string rather than converted to a regular expression.</li>
<li><code>regexp</code> Created by the <code>makeRe</code> method. A single regular expression expressing the entire pattern. This is useful in cases where you wish to use the pattern somewhat like <code>fnmatch(3)</code> with <code>FNM_PATH</code> enabled.</li>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md1466"></a>
...
<h1><a class="anchor" id="autotoc_md1469"></a>
Options</h1>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md1482"></a>
...
<p>If a pattern starts with <code>#</code>, then it is treated as a comment, and will not match anything. Use <code>\#</code> to match a literal <code>#</code> at the start of a line, or set the <code>nocomment</code> flag to suppress this behavior.</p>
...
<p>If an escaped pattern has no matches, and the <code>nonull</code> flag is set, then minimatch.match returns the pattern as-provided, rather than interpreting the character escapes. For example, <code>minimatch.match([], "\\*a\\?")</code> will return <code>"\\*a\\?"</code> rather than <code>"*a?"</code>. This is akin to setting the <code>nullglob</code> option in bash, except that it does not resolve escaped pattern characters.</p>
...
<p>Note that <code>fnmatch(3)</code> in libc is an extremely naive string comparison matcher, which does not do anything special for slashes. This library is designed to be used in glob searching and file walkers, and so it does do special things with <code>/</code>. Thus, <code>foo*</code> will not match <code>foo/bar</code> in this library, even though it would in <code>fnmatch(3)</code>. </p>
</div></div><!-- contents -->
</div><!-- PageDoc -->
</div><!-- doc-content -->
<!-- start footer part -->
