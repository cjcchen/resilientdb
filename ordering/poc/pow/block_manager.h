...
class BlockManager {
 ...
 virtual ~BlockManager() = default;
  ...
  // Obtain the current mining block. Return null if no any blocks.
  Block * GetCurrentBlock();


  std::unique_ptr<BlockMiningInfo> GetPendingBlockInfo();

  int Confirm(const BlockMiningInfo& block_info);
  ...
  bool VerifyBlock(const BlockMiningInfo &mining_info);
