...
/*
	if(new_block->header().height() == 10 && new_block->header().max_seq() - new_block->header().min_seq() +1<= config_.BatchTransactionNum() && shift_idx_ == 0){
		LOG(ERROR)<<"skip fake fail";
		return absl::NotFoundError("solution not found");
	}
	if(new_block->header().height() == 30 && new_block->header().max_seq() - new_block->header().min_seq() +1<= config_.BatchTransactionNum() && shift_idx_ == 0){
		LOG(ERROR)<<"skip fake fail";
		return absl::NotFoundError("solution not found");
	}
  */
  ...
  BlockHeader header(new_block->header());
  ...
  for (const auto& slice : slices) {
    ...
    for (uint32_t i = 0; i < worker_num_; ++i) {
      ...
      ths.push_back(std::thread(
          [&](BlockHeader header, std::pair<uint64_t, uint64_t> slice) {
...
std::string Miner::CalculatePoWHashDigest(const BlockHeader& header) {
  std::string str_value;
...
bool Miner::IsValidHash(const Block* block) {
  LOG(ERROR)<<"block hash:"<< CalculatePoWHash(block).DebugString();
  LOG(ERROR)<<"block value:"<< block->hash().DebugString();
