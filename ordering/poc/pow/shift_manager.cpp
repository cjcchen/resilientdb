...
void ShiftManager::AddSliceInfo(const SliceInfo& slice_info) {
	...
	data_[std::make_pair(slice_info.height(), slice_info.shift_idx())].insert(slice_info.sender());
}
...
bool ShiftManager::Check(const SliceInfo& slice_info, int timeout_ms) {
	auto check_done = [&](){
			...
			return data_[std::make_pair(slice_info.height(), slice_info.shift_idx())].size()>=n-f;
	};
	std::unique_lock<std::mutex> lk(mutex_);
	...
	LOG(ERROR)<<"get shift msg:"<<slice_info.height()<<" "<<slice_info.shift_idx()<<":"<<data_[std::make_pair(slice_info.height(), slice_info.shift_idx())].size();
	return check_done();
}
